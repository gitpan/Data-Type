...
[Note] B<iter()> is a function from the L<Iter> CPAN package (by CPANID B<muenelan> and not the one from L<PerlQT>). It isnt necassary, but instructive.
