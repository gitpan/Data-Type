...
=item Data::Type::Docs::FAQ

Frequently asked questions.

=item Data::Type::Docs::FOP

Frequently occuring problems.

=item Data::Type::Docs::Howto

Point to point recipes how to get things done.

=item Data::Type::Docs::RFC
...
An alternativ "problem encounter" strategie. Instead of returning C<undef> from you subroutine, you C<throw> an exception. Not the strongest point of perl5. But there are some modules on CPAN which try to do the best of it. The one with the nicest syntactic sugar is L<Error> and i use the derived C<Class::Maker::Exception> here. 

=item 'interface'

A collection of methods which are coupled to a package. Some methods have the attribute C<abstract>. This means there is just the name (and idea of what it should do in general) of the method, but you are forced to program the subroutine to fill it with logic. Then you "implemented" the C<abstract> method.
...
=item 'anonymous or private facet'
The implemention of C<summary> required use of facets. Even they arent very reusable by other datatypes. For this you should create <anonymous facets> or C<private facets>. Per convention there names start with '__'. Example:
...
[Note] This module should have been C<Value::Type>, but i stick to C<Data::Type> until i get doomed.
